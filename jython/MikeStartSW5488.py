#Start 5488 train out of SW track 3; returning to SW track 3.
import jarray
import jmri

class StartSW5488(jmri.jmrit.automat.AbstractAutomaton):
    ...
    def handle(self):
        print("This program will start up engine 5488 on SW track 3,")
        print("and circle the layout one time.  It will then back the train into")
        print("SW Staging Track 3.")
        print("==================================================================")
        #Open staging via track 3
        self.openSWTrack3()
        self.throttle = self.getThrottle(5488, True)
        if (self.throttle == None) :
            print ("ERROR: Couldn't assign throttle!")
        else :
            print ("Throttle assigned to locomotive: %s." % 5488)
        print("Starting 5488 train run out of staging")
